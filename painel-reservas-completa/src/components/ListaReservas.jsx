...
export default function ListaReservas({ reservas, salas }) {
  const { alterarStatus } = useReservas();

  function nomeDaSala(salaId) {
    return (
      salas.find((sala) => sala.id === salaId)?.nome ??
      "Sala não encontrada"
    );
  }
  ...
  if (reservas.length === 0) {
    return (
      <p className="vazio">
        Nenhuma reserva corresponde a este filtro.
      </p>
    );
  }
      ...
      {reservas.map((reserva) => (
        <li key={reserva.id}>
          <div>
            ...
            <p>
              {nomeDaSala(reserva.salaId)} · {reserva.horario} ·{" "}
              {reserva.responsavel}
            </p>
          </div>
          ...
          <div className="acoes-reserva">
            ...
            <label>
              <span className="sr-only">
                Status da reserva {reserva.id}
              </span>

              <select
                value={reserva.status}
                onChange={(evento) =>
                  alterarStatus(reserva.id, evento.target.value)
                }
              >
